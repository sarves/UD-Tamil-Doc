...
Mood=Opt   optative
Mood=Pot   potential, if treebank policy uses it
Mood=Nec   necessity, if treebank policy uses it
```
...
Note: Aspect in Tamil is often expressed through auxiliary constructions. Use `Aspect` only if the treebank has a consistent policy for aspectual auxiliaries.
